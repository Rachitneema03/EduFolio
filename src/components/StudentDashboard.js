...
const ProfileContent = ({ userData, onEditProfile, pinnedCertificates, setPinnedCertificates, onNavigateToCertificates }) => {
  ...
  const getHighestAchievedBadge = () => {
    const badges = getBadgeProgress();
    // Find the highest achieved badge (platinum -> gold -> silver -> bronze)
    const achievedBadges = badges.filter(badge => badge.achieved);
    if (achievedBadges.length === 0) return '🏆'; // Default trophy if no badges achieved
    
    // Return the highest badge icon
    const badgeOrder = ['platinum', 'gold', 'silver', 'bronze'];
    for (const badgeId of badgeOrder) {
      const badge = achievedBadges.find(b => b.id === badgeId);
          ...
          <div className="profile-center">
            ...
            <div className="headline-info">
              <div className="headline">
                <div className="headline-label">Headline:</div>
                <span>{userData.profileHeadline || '--'}</span>
              </div>
            </div>
          </div>
